  ...
  <div class="app-container">
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">吐槽对象</label>
          <div class="style-selector">
            <button 
        ...
        <div class="selector-group">
          <label class="selector-label">锐评调性</label>
          <div class="style-selector">
            <button 
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button class="text-link-btn" @click="showGuideModal = true">毒舌锐评与反 PUA 清醒指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
...
const appTitle = ref(appConfig.title || '网腾无限AI - 毒舌锐评与犀利解构专家');
...
const metricsList = [
  {
    "key": "sharpWittyInsight",
    "label": "犀利毒舌洞察力"
  },
  {
    "key": "humorousSatireIndex",
    "label": "幽默讽刺指数"
  },
  {
    "key": "logicalDeconstructionDepth",
    "label": "荒谬逻辑解构深度"
  },
  {
    "key": "viralPunchlinePotential",
    "label": "爆款金句造梗力"
  },
  {
    "key": "truthExposureLevel",
    "label": "真相一针见血度"
  }
];
const guideRules = [
  {
    "title": "用比喻替代说教",
    "advice": "用荒谬生动的比喻（如“把画饼当干粮”）直击本质，比长篇大论的说教更有传播力",
    "avoid": "切忌无厘头的脏话攻击或低俗辱骂"
  },
  {
    "title": "剥开伪装词汇",
    "advice": "识别那些用“沉淀、对齐、组合拳”包装出来的无意义空话，还原其真实成本",
    "avoid": "避免在评价时被对方的高大上词汇带偏逻辑"
  },
  {
    "title": "清醒而不厌世",
    "advice": "锐评的终点是看清真相后依然热爱生活，提供保持边界与内心独立的智慧",
    "avoid": "严禁走向极端消极虚无主义"
  }
];
...
const displayResultText = computed(() => {
  ...
  return result.value.replace(/\[RUIPING_SCORES\][\s\S]*?\[\/RUIPING_SCORES\]/g, '').trim();
});

const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[RUIPING_SCORES\](.*?)\[\/RUIPING_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('ruiping_history_records');
...
const saveHistory = () => {
  localStorage.setItem('ruiping_history_records', JSON.stringify(historyList.value));
};
